...
//#define LOG_NDEBUG 0
#define LOG_TAG "MtkRTPSender"
#include <utils/Log.h>

#include "MtkRTPSender.h"
...
int64_t MtkRTPSender::queuePackets_pre(
...
void MtkRTPSender::queuePackets_pro(
...
status_t MtkRTPSender::calcSendInBitRate(const sp<ABuffer> &buffer)
{
...
status_t MtkRTPSender::calcSendRtpBitRate(const sp<ABuffer> &buffer)
{
    int64_t SendTimeUs = ALooper::GetNowUs();
...
status_t MtkRTPSender::sendRTPPackets(
...
// static
int32_t MtkRTPBase::PickRandomRTPPort()
{
...
status_t MtkRTPSender::initAsync(
...
status_t MtkRTPSender::queueBuffer(
...
status_t MtkRTPSender::queueRawPacket(
    const sp<ABuffer> &packet, uint8_t packetType)
{
    CHECK_LE(packet->size(), (size_t)(kMaxUDPPacketSize - 12));
...
status_t MtkRTPSender::queueTSPackets(
    const sp<ABuffer> &tsPackets, uint8_t packetType)
{
    CHECK_EQ((size_t)0, tsPackets->size() % 188);
...
//    bool  isVideo = isVideoBuffer(tsPackets);
    ...
    while(srcOffset < tsPackets->size()) {
        ...
        //  just used for reconignizing video/audio now
        bool isVideo = refineSSRC(rtp, kSourceID, tsPackets);
...
status_t MtkRTPSender::queueAVCBuffer(
...
status_t MtkRTPSender::sendRTPPacket(
...
// static
uint64_t MtkRTPSender::GetNowNTP()
{
    struct timeval tv;
...
void MtkRTPSender::onMessageReceived(const sp<AMessage> &msg)
{
    switch(msg->what()) {
    case kWhatRTPNotify:
...
void MtkRTPSender::onNetNotify(bool isRTP, const sp<AMessage> &msg)
{
    int32_t reason;
    ...
    case MtkANetworkSession::kWhatDatagram: {
        sp<ABuffer> data;
...
status_t MtkRTPSender::onRTCPData(const sp<ABuffer> &buffer)
{
...
status_t MtkRTPSender::parseReceiverReport(
...
status_t MtkRTPSender::parseTSFB(const uint8_t *data, size_t size)
{
    if((data[0] & 0x1f) != 1) {
...
status_t MtkRTPSender::parseAPP(const uint8_t *data, size_t size)
{
...
void MtkRTPSender::notifyInitDone(status_t err)
{
    sp<AMessage> notify = mNotify->dup();
...
void MtkRTPSender::notifyError(status_t err)
{
    sp<AMessage> notify = mNotify->dup();
...
void MtkRTPSender::notifyNetworkStall(size_t numBytesQueued)
{
    sp<AMessage> notify = mNotify->dup();
