...
namespace android
{
sp<IGraphicExt> gGraphic_service = NULL;
    ...
    mUseAbsoluteTimestamps(false)
{
    ...
    if (gGraphic_service == NULL) {
        gGraphic_service = IGraphicExt::getService();
        if (gGraphic_service == NULL) {
            ALOGD("GraphicBufferUtil failed to get IGraphicExt service\n");
        }
    }
}
...
// Pass the data to the MediaBufferBase. Pass in only the metadata
    ...
    MediaBufferBase **buffer, const ReadOptions * /* options */)
{
    ...
    vendor::mediatek::hardware::gpu::V1_0::ge_sf_info_t ext_info;
    ...
    sprintf(atrace_tag, "SMS-STG");
    {
        ...
        *buffer = NULL;
        ...
        while(mStarted && mNumPendingBuffers == mMaxAcquiredBufferCount) {
            ALOGW("acquired buffer count achieves max, so wait for consumer to release buffer");
            mMediaBuffersAvailableCondition.wait(mMutex);
        }
...
*/
        ...
        if(gGraphic_service != NULL) {
            ...
            ret = gGraphic_service->gralloc_extra_query(mSlots[mCurrentSlot].mGraphicBuffer->handle,
                vendor::mediatek::hardware::gpu::V1_0::GRALLOC_EXTRA_ATTRIBUTE_QUERY::GRALLOC_EXTRA_GET_IOCTL_ION_SF_INFO,
                ...
                if (retval == (int32_t)vendor::mediatek::hardware::gpu::V1_0::GRALLOC_EXTRA_RESULT::GRALLOC_EXTRA_OK) {
                   memcpy(&ext_info, &(dataHandle->data[0]), sizeof(vendor::mediatek::hardware::gpu::V1_0::ge_sf_info_t));
                } else {
                    ALOGD("GraphicBufferUtil failed to extra_query attr %d\n",
                        vendor::mediatek::hardware::gpu::V1_0::GRALLOC_EXTRA_ATTRIBUTE_QUERY::GRALLOC_EXTRA_GET_IOCTL_ION_SF_INFO);
                }
            });

            if(!ret.isOk()){
                ALOGE("gralloc_extra_query failed");
            }
        }
...
// google issue, fixed by MTK temporarily
static buffer_handle_t getMediaBufferHandle(MediaBufferBase *buffer)
{
    VideoNativeMetadata *data = (VideoNativeMetadata *)buffer->data();

    if(data == NULL) {
        ALOGE("NULL metadata buffer data!");
        return 0;
    }
