...
#ifndef MTK_CONVERTER_H_

#define MTK_CONVERTER_H_
...
struct MtkConverter : public AHandler {
    enum {
        kWhatAccessUnit,
        kWhatEOS,
        kWhatError,
    ...
    MtkConverter(const sp<AMessage> &notify,
...
private:
    ...
    void releaseABuffer(sp<ABuffer> &accessUnit);

    DISALLOW_EVIL_CONSTRUCTORS(MtkConverter);
...
private:
    void doMoreWork_p(const sp<ABuffer> &buffer, const sp<MediaCodecBuffer> &outbuf,
                      const int64_t &timeUs, const uint32_t &flags);
    unsigned mPreNalType;

};

}  // namespace android

#endif  // MTK_CONVERTER_H_
