...
///M: export default configuration for WFD
    ...
    if(property_get("vendor.media.wfd.video-format", val, NULL)) {
        ALOGI("vendor.media.wfd.video-format:%s", val);
        int videoFormat = atoi(val);
        MtkVideoFormats::LevelType level = MtkVideoFormats::LEVEL_32;

        property_get("ro.config.low_ram", val, "false");
        if ((0 == strcmp(val, "true")) &&  (videoFormat == 5)) {
            level = MtkVideoFormats::LEVEL_31;
            ALOGI("Force level type for low ram: %d", level);
        }
        ...
        if(videoFormat >=0 && videoFormat <= 16) {
            ...
            mSupportedSourceVideoFormats.setProfileLevel(
                MtkVideoFormats::RESOLUTION_CEA, videoFormat, MtkVideoFormats::PROFILE_CHP, level);
        }
    }
    ...
    if(property_get("vendor.media.wfd.portrait", portrait, NULL)) {
        int value = atoi(portrait);

        if(value == 1) {
            ALOGI("vendor.media.wfd.portrait:%s", portrait);
...
///@}
        ...
        case MtkANetworkSession::kWhatData: {
            ...
            if(property_get("vendor.media.wfd.trigger", val, NULL)) {
...
status_t MtkWifiDisplaySource::sendM3(int32_t sessionID)
{
    ...
    char val[PROPERTY_VALUE_MAX];
    if (property_get("ro.mtk_wfd_hdcp_tx_support", val, NULL)
        && (!strcmp(val, "1"))) {
    ...
    int32_t sessionID, const sp<ParsedMessage> &msg)
{
    ...
    if(!(value == "none")) {
        ...
        if(property_get("ro.vendor.mtk_hybrid_encode_support", val, NULL)
                && (!strcmp(val, "true"))) {
            mChosenVideoProfile = MtkVideoFormats::PROFILE_CBP;
            mChosenVideoLevel = MtkVideoFormats::LEVEL_31;
    ...
    if(!(value == "none")) {
        ...
        if(supportsPCM
                && property_get("vendor.media.wfd.use-pcm-audio", val, NULL)
...
///@}
...
#if 1
        // The older TC1 dongles doesn't specify client_port=xxx apparently.
    ...
    if(mUsingHDCP) {
        ...
        /*playbackSession =
            ...
            mClientPid, mMediaPath.c_str()); remove HDCP*/
    } else {
...
///@}
        ...
        property_set("vendor.media.wfd.met_port", buffer);
#endif
        ///@}
    ...
    const sp<ParsedMessage> &data)
{
    ...
    ///M: @{ For MET integration
#ifdef DEBUG_BUILD
    property_set("vendor.media.wfd.met_port", "-1");
#endif
    ///@}
...
void MtkWifiDisplaySource::setAudioPath(bool on)
{
    ...
    if(on) {
        AudioSystem::setDeviceConnectionState(
            AUDIO_DEVICE_OUT_REMOTE_SUBMIX, AUDIO_POLICY_DEVICE_STATE_AVAILABLE, 0, NULL,AUDIO_FORMAT_DEFAULT);
        AudioSystem::setDeviceConnectionState(
            AUDIO_DEVICE_IN_REMOTE_SUBMIX, AUDIO_POLICY_DEVICE_STATE_AVAILABLE, 0, NULL,AUDIO_FORMAT_DEFAULT);
    } else {
        AudioSystem::setDeviceConnectionState(
            AUDIO_DEVICE_OUT_REMOTE_SUBMIX, AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE, 0, NULL,AUDIO_FORMAT_DEFAULT);
        AudioSystem::setDeviceConnectionState(
            AUDIO_DEVICE_IN_REMOTE_SUBMIX, AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE, 0, NULL,AUDIO_FORMAT_DEFAULT);
    }
}
...
void MtkWifiDisplaySource::notifyThermal(bool start)
{
    ...
    if (start) {
        property_set("vendor.thermal.manager.data.en-policy", "thermal_policy_00");
    } else {
        property_set("vendor.thermal.manager.data.dis-policy", "thermal_policy_00");
    }
}
