...
void MtkRepeaterSource::read_fps(int64_t /*timeUs*/, int64_t readTimeUs)
{
    ...
    if(readTimeUs/1000 > 40 || mWFDFrameLog == true) {
        WFD_LOGI("[WFDP][video]read MediaBufferBase %p,readtime=%lld ms , fps = %.2f", mBuffer,
      ...
      mFrameCount(0)
{
    ...
    if(property_get("vendor.media.wfd.scenario.mode", scenario_en, NULL)) {
        ...
        property_set("vendor.media.wfd.scenario.mode", "1");
    }
        ...
        if(mStopping) {
            if(buffer != NULL) {
                WFD_LOGI("read while stopping buffer=%p,refcnt= %d ",
                         buffer, static_cast<MediaBufferBase *>(buffer)->refcount());
                buffer->release();
                buffer = NULL;
            }
