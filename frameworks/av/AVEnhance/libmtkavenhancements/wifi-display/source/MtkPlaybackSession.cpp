    ...
    MtkVideoFormats::LevelType videoLevelType)
{
    ...
    if(property_get("vendor.media.wfd.drop.dummynal", drop_du, NULL)) {
        if(!strcmp(drop_du, "1")) {
            drop_dummy = true;
        ...
        property_set("vendor.media.wfd.drop.dummynal", "1");
    }
    ...
    //mMediaSender->setHDCP(mHDCP); remove HDCP
    ...
    const sp<AMessage> &msg)
{
    ...
    case kWhatConverterNotify: {
        ...
        if(what == MtkConverter::kWhatAccessUnit) {
            ...
            if(mMiracastEnable || drop_dummy /*|| (mHDCP != NULL) remove HDCP*/) {
                int32_t dummy;
...
MtkWifiDisplaySource::MtkPlaybackSession::MtkPlaybackSession(
    ...
    /*const sp<IHDCP> &hdcp remove HDCP*/ void *hdcp,
    uid_t  ClientUid,
    pid_t  ClientPid,
    const char *path)
