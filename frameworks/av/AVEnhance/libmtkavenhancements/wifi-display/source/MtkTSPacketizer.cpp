...
//#define LOG_NDEBUG 0
#define LOG_TAG "MtkTSPacketizer"
#include <utils/Log.h>

#include "MtkTSPacketizer.h"
...
struct MtkTSPacketizer::Track : public RefBase {
...
void MtkTSPacketizer::Track::extractCSDIfNecessary()
{
    if(mExtractedCSD) {
        return;
    }
...
unsigned MtkTSPacketizer::Track::PID() const
{
    return mPID;
}

unsigned MtkTSPacketizer::Track::streamType() const
{
    return mStreamType;
}

unsigned MtkTSPacketizer::Track::streamID() const
{
    return mStreamID;
}

unsigned MtkTSPacketizer::Track::incrementContinuityCounter()
{
...
bool MtkTSPacketizer::Track::isAudio() const
{
    return !strncasecmp("audio/", mMIME.c_str(), 6);
}

bool MtkTSPacketizer::Track::isVideo() const
{
    return !strncasecmp("video/", mMIME.c_str(), 6);
}

bool MtkTSPacketizer::Track::isH264() const
{
    return !strcasecmp(mMIME.c_str(), MEDIA_MIMETYPE_VIDEO_AVC);
}

bool MtkTSPacketizer::Track::isAAC() const
{
    return !strcasecmp(mMIME.c_str(), MEDIA_MIMETYPE_AUDIO_AAC);
}

bool MtkTSPacketizer::Track::isPCMAudio() const
{
    return !strcasecmp(mMIME.c_str(), MEDIA_MIMETYPE_AUDIO_RAW);
}

bool MtkTSPacketizer::Track::lacksADTSHeader() const
{
    return mAudioLacksATDSHeaders;
}

sp<ABuffer> MtkTSPacketizer::Track::prependCSD(
...
sp<ABuffer> MtkTSPacketizer::Track::prependADTSHeader(
    const sp<ABuffer> &accessUnit) const
{
...
size_t MtkTSPacketizer::Track::countDescriptors() const
{
    return mDescriptors.size();
}

sp<ABuffer> MtkTSPacketizer::Track::descriptorAt(size_t index) const
{
    CHECK_LT(index, mDescriptors.size());
    return mDescriptors.itemAt(index);
}

void MtkTSPacketizer::Track::finalize()
{
    if(mFinalized) {
        return;
    }
    ...
    if(isH264()) {
        {
            ...
            if(mCSD.size() > 0) {
                ...
                CHECK_GE((size_t)profileIdc, 0u);
                CHECK_GE((size_t)levelIdc, 0u);
...
MtkTSPacketizer::MtkTSPacketizer(uint32_t flags)
    : mFlags(flags),
...
MtkTSPacketizer::~MtkTSPacketizer()
{
}

ssize_t MtkTSPacketizer::addTrack(const sp<AMessage> &format)
{
    AString mime;
...
status_t MtkTSPacketizer::extractCSDIfNecessary(size_t trackIndex)
{
...
status_t MtkTSPacketizer::packetize(
    size_t trackIndex,
    ...
    */
    ...
    if(flags & EMIT_PAT_AND_PMT) {
        ...
        *ptr++ = kPID_PCR & 0xff;
        ...
        CHECK_LT(program_info_length, (size_t)0x400);
        ...
        *ptr++ = (program_info_length & 0xff);
        ...
        for(size_t i = 0; i < mTracks.size(); ++i) {
            ...
            *ptr++ = track->PID() & 0xff;
            ...
            CHECK_LE(ES_info_length, (size_t)0xfff);
...
void MtkTSPacketizer::initCrcTable()
{
...
uint32_t MtkTSPacketizer::crc32(const uint8_t *start, size_t size) const
{
...
sp<ABuffer> MtkTSPacketizer::prependCSD(
