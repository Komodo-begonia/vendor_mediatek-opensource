...
// static
sp<MtkParameters> MtkParameters::Parse(const char *data, size_t size)
{
    sp<MtkParameters> params = new MtkParameters;
...
MtkParameters::MtkParameters() {}

MtkParameters::~MtkParameters() {}

status_t MtkParameters::parse(const char *data, size_t size)
{
    size_t i = 0;
...
bool MtkParameters::findParameter(const char *name, AString *value) const
{
    AString key = name;
    key.tolower();
