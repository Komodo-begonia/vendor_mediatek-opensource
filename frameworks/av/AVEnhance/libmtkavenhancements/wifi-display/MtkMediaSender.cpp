...
bool MtkMediaSender::allTracksHaveAccessUnit()
{
      ...
      mLogFile(NULL)
{
    ...
    if(property_get("vendor.media.wfd.av.sync", avsync_en, NULL)) {
        ...
        property_set("vendor.media.wfd.av.sync", "1");
    }
...
/* remove HDCP
status_t MtkMediaSender::setHDCP(const sp<IHDCP> &hdcp)
{
...
ssize_t MtkMediaSender::addTrack(const sp<AMessage> &format, uint32_t flags)
{
    if(mMode != MODE_UNDEFINED) {
        return INVALID_OPERATION;
    }
...
status_t MtkMediaSender::initAsync(
    ssize_t trackIndex,
    ...
    int32_t *localRTPPort)
{
    if(trackIndex < 0) {
        ...
        if(mHDCP != NULL) {
            ...
            flags |= MtkTSPacketizer::EMIT_HDCP20_DESCRIPTOR;
        }*/

        mTSPacketizer = new MtkTSPacketizer(flags);
        ...
        if(err == OK) {
            ...
            mTSSender = new MtkRTPSender(mNetSession, notify);
    ...
    info->mSender = new MtkRTPSender(mNetSession, notify);
...
status_t MtkMediaSender::queueAccessUnit(
    size_t trackIndex, const sp<ABuffer> &accessUnit)
{
    ...
    if(mMode == MODE_TRANSPORT_STREAM) {
        ...
        if(avsync_enable == true) {
            if((!(info->mIsAudio)) && (mFirstVideoTimeUs == -1)) {
                CHECK(accessUnit->meta()->findInt64("timeUs", &mFirstVideoTimeUs));
                ALOGD("mFirstVideoTimeUs=%lld", (long long)mFirstVideoTimeUs);
            }

            //  for sumsung tv IOT ALPS03105046
            if(info->mIsAudio && (mFirstVideoTimeUs != -1)) {
                int64_t AudioTimeUs;
                CHECK(accessUnit->meta()->findInt64("timeUs", &AudioTimeUs));

                ALOGV("mFirstVideoTimeUs=%lld AudioTimeUs=%lld",
                      (long long)mFirstVideoTimeUs, (long long)AudioTimeUs);

                if(AudioTimeUs < mFirstVideoTimeUs) {
                    info->mAccessUnits.erase(info->mAccessUnits.begin());
                    ALOGD("mFirstVideoTimeUs(%lld) > AudioTimeUs(%lld) drop",
                          (long long)mFirstVideoTimeUs, (long long)AudioTimeUs);
                }
            }
        }
...
//  send any data ready, avoid one track wait for another track , which will cause data blcok here
            ...
            if(!accessUnit->meta()->findInt32("dummy-nal", &dummy)) {
                //  ALOGD("dummy not found in MtkMediaSender");
                dummy = 0;
            } else {
                //  ALOGD("find dummy");
            }

            if(!accessUnit->meta()->findInt64("latencyB", &latencyB)) {
                latencyB = -1;
            }

            if(!accessUnit->meta()->findInt64("LatencyToken", &LatencyToken)) {
                LatencyToken = -1;
            }
            ...
            if(err == OK) {
                ...
                err = mTSSender->queueBuffer(
                          tsPackets,
                          33 /* packetType */,
                          MtkRTPSender::PACKETIZATION_TRANSPORT_STREAM);
            }
    ...
    return info->mSender->queueBuffer(
               ...
               info->mIsAudio
               ? MtkRTPSender::PACKETIZATION_AAC : MtkRTPSender::PACKETIZATION_H264);
}

void MtkMediaSender::onMessageReceived(const sp<AMessage> &msg)
{
    switch(msg->what()) {
...
void MtkMediaSender::onSenderNotify(const sp<AMessage> &msg)
{
    int32_t what;
    CHECK(msg->findInt32("what", &what));

    switch(what) {
    case MtkRTPSender::kWhatInitDone: {
        --mInitDoneCount;
...
void MtkMediaSender::notifyInitDone(status_t err)
{
    sp<AMessage> notify = mNotify->dup();
...
void MtkMediaSender::notifyError(status_t err)
{
    sp<AMessage> notify = mNotify->dup();
...
void MtkMediaSender::notifyNetworkStall(size_t numBytesQueued)
{
    sp<AMessage> notify = mNotify->dup();
...
status_t MtkMediaSender::packetizeAccessUnit(
    size_t trackIndex,
    ...
    if(mHDCP != NULL && !info.mIsAudio) {
        ...
        if(accessUnit->meta()->findPointer("handle", (void**)&handle)
            ...
            CHECK(accessUnit->meta()->findMessage("notify", &notify)
                  && notify != NULL);
            CHECK_GE(accessUnit->size(), (size_t)rangeLength);
...
//  should do it when "handle" mode
        ...
        flags |= MtkTSPacketizer::IS_ENCRYPTED;
        ...
        if(!accessUnit->meta()->findInt32("dummy-nal", &dummy)) {
            ...
            if(costTimeUs > 10000) {
                ALOGW("hdcp cost %lldms", (long long)costTimeUs/1000);
            }
        }
...
#endif
        if(manuallyPrependSPSPPS) {
        flags |= MtkTSPacketizer::PREPEND_SPS_PPS_TO_IDR_FRAMES;
        }
#if 0
    remove HDCP
    }
    ...
    if(mPrevTimeUs < 0ll || mPrevTimeUs + 100000ll <= timeUs) {
        flags |= MtkTSPacketizer::EMIT_PCR;
        flags |= MtkTSPacketizer::EMIT_PAT_AND_PMT;
