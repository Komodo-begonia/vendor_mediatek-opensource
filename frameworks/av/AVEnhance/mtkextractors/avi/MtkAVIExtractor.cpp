...
class MtkAVISource : public MediaTrackHelper {
    public:
        ...
        virtual const char * extractorName() { return "MtkAVIExtractor"; }
    protected:
...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, MtkAVISource* source) {
    ...
    if (source->mIsVideo) {
        ...
        if (source->mIsDIVX || source->mIsDIVX3) {
            if (!hasDivxLicense()) {
                ALOGE("skip divx(%s) video because of no divx license", mime);
                source->mIsVideo = false;
            }
        }
...
media_status_t MtkAVISource::stop() {
    Mutex::Autolock autoLock(mLock);
     ...
     if (!mStarted) {
        ALOGD("AVI mIsAudio %d Source already stopped", mIsAudio);
        return AMEDIA_OK;
    }
...
bool hasDivxLicense() {
    ...
    if (err != OK) {
        ALOGE("hasDivxLicense client connect fail err:%d\n", err);
        return ret;
    }
    ...
    const char* name = "OMX.MTK.VIDEO.DECODER.DIVX";
    const char *role = "video_decoder.divx";

    err = omx->allocateNode(name, observer, &omxNode);
    if (err != OK) {
        ALOGE("hasDivxLicense allocateNode fail\n");
        client.disconnect();
        return ret;
    }
    ...
    if (err != OK) {
        ALOGE("hasDivxLicense SetComponentRole fail\n");
        omxNode->freeNode();
        client.disconnect();
        return ret;
    }
    ...
    if (omxNode == NULL) {
        ALOGE("hasDivxLicense omxNode null\n");
        return ret;
    }
    ...
    param.nProfileIndex = -1; /*special handling that nProfileIndex = -1 for query divx license*/
    err = omxNode->getParameter(
            OMX_IndexParamVideoProfileLevelQuerySupported,
            &param, sizeof(param));
    if (err != OK) {
        ALOGE("hasDivxLicense getParameter err:%d\n", err);
        return ret;
    }
    //}
