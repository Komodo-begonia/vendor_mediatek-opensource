...
public:
    explicit FLVExtractor(DataSourceHelper *source);
    virtual size_t countTracks();
    virtual MediaTrackHelper *getTrack(size_t index);
    virtual media_status_t getTrackMetaData(AMediaFormat *meta, size_t index, uint32_t flags);
    virtual media_status_t getMetaData(AMediaFormat *meta);
    FLV_ERROR_TYPE flvMakeHEVCCodecSpecificData(const sp<ABuffer> &buffer, AMediaFormat *meta);
    virtual uint32_t flags() const;
    virtual const char * name() { return "MtkFLVExtractor"; }
...
public:
    ...
    MediaBufferHelper* DequeueVideoFrame(int64_t targetSampleTimeUs = -1);
    MediaBufferHelper* DequeueAudioFrame(int64_t targetSampleTimeUs = -1);
