...
//#define LOG_NDEBUG 0
#define LOG_TAG "MtkMP3Extractor"
#include <utils/Log.h>

#include "MtkMP3Extractor.h"
...
class MP3Source : public MediaTrackHelper , public MtkMP3Toc {
public:
    MP3Source(
            MP3Extractor *extractor,  //mtkadd for mp3 low power
...
//mtkadd+
    MP3Extractor *mExtractor;
    bool mMp3UseMultiRead;
    bool mMP3UseMultiBuffer;
    bool mEnableTOC;  //for toc thread
//mtkadd-
};
...
MP3Extractor::MP3Extractor(
    ...
    : mInitCheck(NO_INIT),
      ...
      mFixedHeader(0),
      mSeeker(NULL) {
    mUseMultiBufferFlag = false;
    mCodecLowPowerFlag = false;
...
MediaTrackHelper *MP3Extractor::getTrack(size_t index) {
    ...
    return new MP3Source(this, //mtkadd to set extractor to MP3Source
...
#if 1
//mtkadd+
    //flags 0xffffffff: codec set sucessfully ;
    //0 < flags < 0xffffffff: set App PID to mp3extractor
    if (flags == 0xffffffff) {
        setCodecLowPowerFlag(true);
        ALOGV("set mp3 low power flags true.");
    } else if (flags > 0) {
        if (isFindMtkMp3Codec(MEDIA_MIMETYPE_AUDIO_MPEG, false)
                && isMtkMp3Music(flags)) {
            setMp3UseMultiBufferFlag(true);
            ALOGV("setMp3UseMultiBufferFlag true.");
        } else {
            ALOGW("it's not MtkMp3Extractor expected music or codec!");
        }
    }
//mtkadd-
#endif
...
MP3Source::MP3Source(
    ...
    : MtkMP3Toc(source, fixed_header),
      ...
      mBasisTimeUs(0),
      mSamplesRead(0),
      mExtractor(extractor),  //mtkadd
      mMp3UseMultiRead(false), //mtkadd
      mMP3UseMultiBuffer(false) {  //mtkadd
...
#if 1
        //mtkadd+
    if (mExtractor != NULL && mExtractor->getMp3UseMultiBufferFlag()) {
        mBufferGroup->add_buffer(kMaxFrameSize* MULTI_FRAME);
        mMP3UseMultiBuffer = true;
    } else {
...
media_status_t MP3Source::read(
    ...
    *out = NULL;
#if 1
    if (!mMp3UseMultiRead && mExtractor != NULL) {
        mMp3UseMultiRead = mMP3UseMultiBuffer && mExtractor->getCodecLowPowerFlag();
        if (mMp3UseMultiRead) {
            ALOGD("mMp3UseMultiRead is true!");
        }
    }
#endif
    ...
    if (options != NULL && options->getSeekTo(&seekTimeUs, &mode)) {
        ...
        if (mEnableTOC) {
            ...
            ALOGD("getFramePos by toc, seekTimeUs(%lld), durationUsByToc(%lld)",
                    (long long)seekTimeUs, (long long)durationUsByTOC);
...
//mtkadd+
bool MP3Extractor::getMp3UseMultiBufferFlag() {
    Mutex::Autolock autoLock(mLock);
    return mUseMultiBufferFlag;
}

void MP3Extractor::setMp3UseMultiBufferFlag(bool flag) {
    Mutex::Autolock autoLock(mLock);
    mUseMultiBufferFlag = flag;
}

bool MP3Extractor::getCodecLowPowerFlag() {
    Mutex::Autolock autoLock(mLock);
    return mCodecLowPowerFlag;
}

void MP3Extractor::setCodecLowPowerFlag(bool flag) {
    Mutex::Autolock autoLock(mLock);
    mCodecLowPowerFlag = flag;
}
//mtkadd-
...
static CreatorFunc Sniff(
    ...
    *freeMeta = ::free;
    ...
    //*confidence = 0.2f;       //google default confidence
    *confidence = 0.25f;  //mtk confidence
...
ExtractorDef GETEXTRACTORDEF() {
    return {
        ...
        //UUID("812a3f6c-c8cf-46de-b529-3774b14103d4"),   //google default
        UUID("c22dbec7-2d6b-48a0-ad49-25991a8a0d2c"),   //mtk
        1, // version
        "MtkMP3 Extractor",
        { .v3 = {Sniff, extensions} }
    };
