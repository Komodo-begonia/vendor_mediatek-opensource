...
//#define LOG_NDEBUG 0
#define LOG_TAG "MtkATSParser"
#include <utils/Log.h>
#include "MtkATSParser.h"
#include "MtkAnotherPacketSource.h"
#include "MtkCasManager.h"
...
ATSParser::Stream::Stream(
    ...
    if (((isVideo() || isAudio()) && mScrambled)) {
        flags = mtk::ElementaryStreamQueue::kFlag_ScrambledData;
    } else if (mSampleEncrypted) {
        flags = mtk::ElementaryStreamQueue::kFlag_SampleEncryptedData;
    }

    mtk::ElementaryStreamQueue::Mode mode = mtk::ElementaryStreamQueue::INVALID;

    switch (mStreamType) {
        case STREAMTYPE_H264:
        case STREAMTYPE_H264_ENCRYPTED:
            mode = mtk::ElementaryStreamQueue::H264;
            flags |= (mProgram->parserFlags() & ALIGNED_VIDEO_DATA) ?
                    mtk::ElementaryStreamQueue::kFlag_AlignedData : 0;
            break;

        case STREAMTYPE_MPEG2_AUDIO_ADTS:
        case STREAMTYPE_AAC_ENCRYPTED:
            mode = mtk::ElementaryStreamQueue::AAC;
            break;

        case STREAMTYPE_MPEG1_AUDIO:
        case STREAMTYPE_MPEG2_AUDIO:
            mode = mtk::ElementaryStreamQueue::MPEG_AUDIO;
            break;

        case STREAMTYPE_MPEG1_VIDEO:
        case STREAMTYPE_MPEG2_VIDEO:
            mode = mtk::ElementaryStreamQueue::MPEG_VIDEO;
            break;

        case STREAMTYPE_MPEG4_VIDEO:
            mode = mtk::ElementaryStreamQueue::MPEG4_VIDEO;
            break;

        case STREAMTYPE_LPCM_AC3:
        case STREAMTYPE_AC3:
        case STREAMTYPE_AC3_ENCRYPTED:
            mode = mtk::ElementaryStreamQueue::AC3;
            break;

        case STREAMTYPE_EAC3:
            mode = mtk::ElementaryStreamQueue::EAC3;
            break;

        case STREAMTYPE_PES_PRIVATE_DATA:
            if (mStreamTypeExt == EXT_DESCRIPTOR_DVB_AC4) {
                mode = mtk::ElementaryStreamQueue::AC4;
            }
            break;

        case STREAMTYPE_METADATA:
            mode = mtk::ElementaryStreamQueue::METADATA;
            break;
        case STREAMTYPE_HEVC:
            mode = mtk::ElementaryStreamQueue::HEVC;
            break;
    ...
    mQueue = new mtk::ElementaryStreamQueue(mode, flags);
    ...
    if (mQueue != NULL) {
        ...
        if (mScrambled && (isAudio() || isVideo())) {
            ...
            mSource = new mtk::AnotherPacketSource(meta);
        }
    }
}
...
//  duration patch --
    ...
    if (mLastTimeUs != -1 && (timeUs > mLastTimeUs &&
        (timeUs - mLastTimeUs > 10000000))) {
        ALOGD("timeUs %lld - mLastTimeUs %lld > 10s, reset timeUs",
            (long long)timeUs, (long long)mLastTimeUs);
        timeUs = mLastTimeUs;
    } else {
        mLastTimeUs = timeUs;
    }
    ...
    if (mSource != NULL && mSource->getQueuedBufferSize() > 1000*1024*1024) {
        ALOGV("ASource queue too many buffer,stop queueAccessUnit");
        return;
    }
            ...
            if (meta != NULL) {
                ...
                if (meta->findCString(kKeyMIMEType, &mime)
                    ...
                    if (!accessUnit->meta()->findInt32("isSync", &sync) || !sync) {
                        int32_t ISlice = 0;
                        if (accessUnit->meta()->findInt32("isISlice", &ISlice) && ISlice) {
                            ALOGD("find I slice");
                        } else {
                          continue;
                        }
